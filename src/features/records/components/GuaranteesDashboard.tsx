...
import { Loader2, Search, Filter, Eye, X, User, MapPin } from "lucide-react";
...
export default function GuaranteesDashboard() {
  ...
  const filteredWarranties = useMemo(() => {
    ...
    return warranties.filter((item: any) => {
      const matchesSearch = 
  ...
  const handleOpenModal = (warranty: any) => {
            ...
            <TableBody>
              {filteredWarranties.map((item: any) => (
                ...
                >
                  ...
                  <TableCell>
                    <Badge variant="outline" className={`capitalize border shadow-sm ${
                      item.estado === 'pendiente' ? 'bg-yellow-50 text-yellow-700 border-yellow-200' :

import { i as __toESM, n as __commonJS, t as require_react } from "./react-BMSG9V3O.js";
import { t as require_react_dom } from "./react-dom-Dozigys7.js";

//#region node_modules/.pnpm/react-router@7.9.6_react-do_d4a3e4c27e0cb30e84e7f4e67d54cb39/node_modules/react-router/dist/development/chunk-4WY6JWTD.mjs
...
function createMemoryHistory(options = {}) {
	...
	function createMemoryLocation(to, state = null, key) {
		let location$1 = createLocation(entries ? getCurrentLocation().pathname : "/", to, state, key);
		warning(location$1.pathname.charAt(0) === "/", `relative pathnames are not supported in memory history: ${JSON.stringify(to)}`);
		return location$1;
	}
...
function createHashHistory(options = {}) {
	...
	function createHashHref(window2, to) {
		...
		if (base && base.getAttribute("href")) {
			...
			href$1 = hashIndex === -1 ? url : url.slice(0, hashIndex);
		}
		return href$1 + "#" + (typeof to === "string" ? to : createPath(to));
	}
	function validateHashLocation(location$1, to) {
		warning(location$1.pathname.charAt(0) === "/", `relative pathnames are not supported in hash history.push(${JSON.stringify(to)})`);
	}
...
function getHistoryState(location$1, index) {
	return {
		usr: location$1.state,
		key: location$1.key,
		idx: index
	};
}
...
function getUrlBasedHistory(getLocation, createHref2, validateLocation, options = {}) {
	...
	function push(to, state) {
		action = "PUSH";
		let location$1 = createLocation(history.location, to, state);
		if (validateLocation) validateLocation(location$1, to);
		index = getIndex() + 1;
		let historyState = getHistoryState(location$1, index);
		let url = history.createHref(location$1);
		try {
...
function createBrowserURLImpl(to, isAbsolute = false) {
	...
	if (!isAbsolute && href$1.startsWith("//")) href$1 = base + href$1;
...
function stripBasename(pathname, basename) {
	...
	let startIndex$1 = basename.endsWith("/") ? basename.length - 1 : basename.length;
	let nextChar = pathname.charAt(startIndex$1);
...
function instrumentClientSideRouter(router$1, fns) {
	let aggregated = {
		navigate: [],
		fetch: []
	};
	...
	if (aggregated.navigate.length > 0) {
		let navigate = router$1.navigate[UninstrumentedSymbol] ?? router$1.navigate;
		let instrumentedNavigate = wrapImpl(aggregated.navigate, navigate, (...args) => {
			let [to, opts] = args;
			return {
				to: typeof to === "number" || typeof to === "string" ? to : to ? createPath(to) : ".",
				...getRouterInfo(router$1, opts ?? {})
			};
		});
		if (instrumentedNavigate) {
			instrumentedNavigate[UninstrumentedSymbol] = navigate;
			router$1.navigate = instrumentedNavigate;
		}
	}
	if (aggregated.fetch.length > 0) {
		let fetch2 = router$1.fetch[UninstrumentedSymbol] ?? router$1.fetch;
		let instrumentedFetch = wrapImpl(aggregated.fetch, fetch2, (...args) => {
			let [key, , href$1, opts] = args;
			return {
				href: href$1 ?? ".",
				fetcherKey: key,
				...getRouterInfo(router$1, opts ?? {})
		...
		if (instrumentedFetch) {
			...
			router$1.fetch = instrumentedFetch;
		}
	}
	return router$1;
}
...
function getRouterInfo(router$1, opts) {
	return {
		currentUrl: createPath(router$1.state.location),
...
function createRouter(init) {
	...
	function initialize() {
		unlistenHistory = init.history.listen(({ action: historyAction, location: location$1, delta }) => {
			...
			return startNavigation(historyAction, location$1);
		});
		if (isBrowser2) {
	...
	function completeNavigation(location$1, newState, { flushSync } = {}) {
		let isActionReload = state.actionData != null && state.navigation.formMethod != null && isMutationMethod(state.navigation.formMethod) && state.navigation.state === "loading" && location$1.state?._isRedirect !== true;
		let actionData;
		...
		let restoreScrollPosition = isUninterruptedRevalidation ? false : getSavedScrollPosition(location$1, newState.matches || state.matches);
		let preventScrollReset = pendingPreventScrollReset === true || state.navigation.formMethod != null && isMutationMethod(state.navigation.formMethod) && location$1.state?._isRedirect !== true;
		if (inFlightDataRoutes) {
			dataRoutes = inFlightDataRoutes;
			inFlightDataRoutes = void 0;
		}
		if (isUninterruptedRevalidation) {} else if (pendingAction === "POP") {} else if (pendingAction === "PUSH") init.history.push(location$1, location$1.state);
		else if (pendingAction === "REPLACE") init.history.replace(location$1, location$1.state);
		let viewTransitionOpts;
		if (pendingAction === "POP") {
			let priorPaths = appliedViewTransitions.get(state.location.pathname);
			if (priorPaths && priorPaths.has(location$1.pathname)) viewTransitionOpts = {
				currentLocation: state.location,
				nextLocation: location$1
			};
			else if (appliedViewTransitions.has(location$1.pathname)) viewTransitionOpts = {
				currentLocation: location$1,
				nextLocation: state.location
			};
		} else if (pendingViewTransitionEnabled) {
			let toPaths = appliedViewTransitions.get(state.location.pathname);
			if (toPaths) toPaths.add(location$1.pathname);
			else {
				toPaths = /* @__PURE__ */ new Set([location$1.pathname]);
	...
	async function startNavigation(historyAction, location$1, opts) {
		pendingNavigationController && pendingNavigationController.abort();
		...
		let matches = opts?.initialHydration && state.matches && state.matches.length > 0 && !initialMatchesIsFOW ? state.matches : matchRoutes(routesToUse, location$1, basename);
		let flushSync = (opts && opts.flushSync) === true;
		if (matches && state.initialized && !isRevalidationRequired && isHashChangeOnly(state.location, location$1) && !(opts && opts.submission && isMutationMethod(opts.submission.formMethod))) {
			completeNavigation(location$1, { matches }, { flushSync });
			return;
		}
		let fogOfWar = checkFogOfWar(matches, routesToUse, location$1.pathname);
		if (fogOfWar.active && fogOfWar.matches) matches = fogOfWar.matches;
		if (!matches) {
			let { error, notFoundMatches, route } = handleNavigational404(location$1.pathname);
			completeNavigation(location$1, {
		...
		let request = createClientSideRequest(init.history, location$1, pendingNavigationController.signal, opts && opts.submission);
		...
		if (opts && opts.pendingError) pendingActionResult = [findNearestBoundary(matches).route.id, {
			...
			let actionResult = await handleAction(request, location$1, opts.submission, matches, scopedContext, fogOfWar.active, opts && opts.initialHydration === true, {
			...
			loadingNavigation = getLoadingNavigation(location$1, opts.submission);
			flushSync = false;
			fogOfWar.active = false;
			request = createClientSideRequest(init.history, request.url, request.signal);
		}
		let { shortCircuited, matches: updatedMatches, loaderData, errors } = await handleLoaders(request, location$1, matches, scopedContext, fogOfWar.active, loadingNavigation, opts && opts.submission, opts && opts.fetcherSubmission, opts && opts.replace, opts && opts.initialHydration === true, flushSync, pendingActionResult);
	...
	async function handleAction(request, location$1, submission, matches, scopedContext, isFogOfWar, initialHydration, opts = {}) {
		interruptActiveLoads();
		updateState({ navigation: getSubmittingNavigation(location$1, submission) }, { flushSync: opts.flushSync === true });
		if (isFogOfWar) {
			let discoverResult = await discoverRoutes(matches, location$1.pathname, request.signal);
			if (discoverResult.type === "aborted") return { shortCircuited: true };
				...
				let { notFoundMatches, error, route } = handleNavigational404(location$1.pathname);
				return {
					matches: notFoundMatches,
					pendingActionResult: [route.id, {
						type: "error",
		...
		let result;
		let actionMatch = getTargetMatch(matches, location$1);
		...
		if (isRedirectResult(result)) {
			...
			else replace2 = normalizeRedirectLocation(result.response.headers.get("Location"), new URL(request.url), basename) === state.location.pathname + state.location.search;
	...
	async function handleLoaders(request, location$1, matches, scopedContext, isFogOfWar, overrideNavigation, submission, fetcherSubmission, replace2, initialHydration, flushSync, pendingActionResult) {
		let loadingNavigation = overrideNavigation || getLoadingNavigation(location$1, submission);
		...
		let routesToUse = inFlightDataRoutes || dataRoutes;
		let { dsMatches, revalidatingFetchers } = getMatchesToLoad(request, scopedContext, mapRouteProperties2, manifest, init.history, state, matches, activeSubmission, location$1, initialHydration ? [] : hydrationRouteProperties2, initialHydration === true, isRevalidationRequired, cancelledFetcherLoads, fetchersQueuedForDeletion, fetchLoadMatches, fetchRedirectIds, routesToUse, basename, init.patchRoutesOnNavigation != null, pendingActionResult);
	...
	async function fetch2(key, routeId, href$1, opts) {
		abortFetcher(key);
		let flushSync = (opts && opts.flushSync) === true;
		let routesToUse = inFlightDataRoutes || dataRoutes;
		let normalizedPath = normalizeTo(state.location, state.matches, basename, href$1, routeId, opts?.relative);
	...
	async function startRedirectNavigation(request, redirect2, isNavigation, { submission, fetcherSubmission, preventScrollReset, replace: replace2 } = {}) {
		...
		let location$1 = redirect2.response.headers.get("Location");
		invariant(location$1, "Expected a Location header on the redirect Response");
		location$1 = normalizeRedirectLocation(location$1, new URL(request.url), basename);
		let redirectLocation = createLocation(state.location, location$1, { _isRedirect: true });
		if (isBrowser2) {
			let isDocumentReload = false;
			if (redirect2.response.headers.has("X-Remix-Reload-Document")) isDocumentReload = true;
			else if (isAbsoluteUrl(location$1)) {
				const url = createBrowserURLImpl(location$1, true);
				isDocumentReload = url.origin !== routerWindow.location.origin || stripBasename(url.pathname, basename) == null;
			}
			if (isDocumentReload) {
				if (replace2) routerWindow.location.replace(location$1);
				else routerWindow.location.assign(location$1);
				return;
			}
		}
		pendingNavigationController = null;
	...
	function getScrollKey(location$1, matches) {
		if (getScrollRestorationKey2) return getScrollRestorationKey2(location$1, matches.map((m) => convertRouteMatchToUiMatch(m, state.loaderData))) || location$1.key;
		return location$1.key;
	}
	function saveScrollPosition(location$1, matches) {
		if (savedScrollPositions2 && getScrollPosition) {
			let key = getScrollKey(location$1, matches);
			savedScrollPositions2[key] = getScrollPosition();
		}
	}
	function getSavedScrollPosition(location$1, matches) {
	...
	if (init.unstable_instrumentations) router$1 = instrumentClientSideRouter(router$1, init.unstable_instrumentations.map((i) => i.router).filter(Boolean));
	return router$1;
}
function createStaticHandler(routes, opts) {
	...
	async function query(request, { requestContext, filterMatchesToLoad, skipLoaderErrorBubbling, skipRevalidation, dataStrategy, generateMiddlewareResponse } = {}) {
		...
		let location$1 = createLocation("", createPath(url), null, "default");
		let matches = matchRoutes(dataRoutes, location$1, basename);
		...
		if (!isValidMethod(method) && method !== "HEAD") {
			...
			let error = getInternalRouterError(404, { pathname: location$1.pathname });
			...
			try {
				...
				let response = await runServerMiddlewarePipeline({
					...
					return await generateMiddlewareResponse(async (revalidationRequest, opts2 = {}) => {
						let result2 = await queryImpl(revalidationRequest, location$1, matches, requestContext, dataStrategy || null, skipLoaderErrorBubbling === true, null, "filterMatchesToLoad" in opts2 ? opts2.filterMatchesToLoad ?? null : filterMatchesToLoad ?? null, skipRevalidation === true);
		...
		let result = await queryImpl(request, location$1, matches, requestContext, dataStrategy || null, skipLoaderErrorBubbling === true, null, filterMatchesToLoad || null, skipRevalidation === true);
	...
	async function queryRoute(request, { routeId, requestContext, dataStrategy, generateMiddlewareResponse } = {}) {
		...
		else if (!matches) throw getInternalRouterError(404, { pathname: location$1.pathname });
		let match = routeId ? matches.find((m) => m.route.id === routeId) : getTargetMatch(matches, location$1);
		if (routeId && !match) throw getInternalRouterError(403, {
			pathname: location$1.pathname,
			routeId
		});
		else if (!match) throw getInternalRouterError(404, { pathname: location$1.pathname });
		if (generateMiddlewareResponse) {
			...
			return await runServerMiddlewarePipeline({
				...
				return await generateMiddlewareResponse(async (innerRequest) => {
					let processed = handleQueryResult(await queryImpl(innerRequest, location$1, matches, requestContext, dataStrategy || null, false, match, null, false));
		...
		return handleQueryResult(await queryImpl(request, location$1, matches, requestContext, dataStrategy || null, false, match, null, false));
	...
	async function queryImpl(request, location$1, matches, requestContext, dataStrategy, skipLoaderErrorBubbling, routeMatch, filterMatchesToLoad, skipRevalidation) {
		invariant(request.signal, "query()/queryRoute() requests must contain an AbortController signal");
		try {
			if (isMutationMethod(request.method)) return await submit(request, matches, routeMatch || getTargetMatch(matches, location$1), requestContext, dataStrategy, skipLoaderErrorBubbling, routeMatch != null, filterMatchesToLoad, skipRevalidation);
...
function normalizeTo(location$1, matches, basename, to, fromRouteId, relative) {
	...
	let path = resolveTo(to ? to : ".", getResolveToMatches(contextualMatches), stripBasename(location$1.pathname, basename) || location$1.pathname, relative === "path");
	if (to == null) {
		path.search = location$1.search;
		path.hash = location$1.hash;
	}
...
function getMatchesToLoad(request, scopedContext, mapRouteProperties2, manifest, history, state, matches, submission, location$1, lazyRoutePropertiesToSkip, initialHydration, isRevalidationRequired, cancelledFetcherLoads, fetchersQueuedForDeletion, fetchLoadMatches, fetchRedirectIds, routesToUse, basename, hasPatchRoutesOnNavigation, pendingActionResult) {
	...
	let nextUrl = history.createURL(location$1);
	let maxIdx;
	...
	let dsMatches = matches.map((match, index) => {
		...
		return getDataStrategyMatch(mapRouteProperties2, manifest, request, pattern, match, lazyRoutePropertiesToSkip, scopedContext, shouldRevalidateLoader(match, shouldRevalidateArgs), shouldRevalidateArgs);
	});
...
function runClientMiddlewarePipeline(args, handler) {
	...
	function errorHandler(error, routeId, nextResult) {
		if (nextResult) return Promise.resolve(Object.assign(nextResult.value, { [routeId]: {
			...
			let { matches } = args;
			let boundaryRouteId = findNearestBoundary(matches, matches[Math.min(Math.max(matches.findIndex((m) => m.route.id === routeId), 0), Math.max(matches.findIndex((m) => m.unstable_shouldCallHandler()), 0))].route.id).route.id;
...
function normalizeRelativeRoutingRedirectResponse(response, request, routeId, matches, basename) {
	let location$1 = response.headers.get("Location");
	invariant(location$1, "Redirects returned/thrown from loaders/actions must have a Location header");
	if (!isAbsoluteUrl(location$1)) {
		let trimmedMatches = matches.slice(0, matches.findIndex((m) => m.route.id === routeId) + 1);
		location$1 = normalizeTo(new URL(request.url), trimmedMatches, basename, location$1);
		response.headers.set("Location", location$1);
	}
	return response;
}
function normalizeRedirectLocation(location$1, currentUrl, basename) {
	if (isAbsoluteUrl(location$1)) {
		let normalizedLocation = location$1;
...
function createClientSideRequest(history, location$1, signal, submission) {
	let url = history.createURL(stripHashFromPath(location$1)).toString();
	let init = { signal };
	if (submission && isMutationMethod(submission.formMethod)) {
...
function getTargetMatch(matches, location$1) {
	let search = typeof location$1 === "string" ? parsePath(location$1).search : location$1.search;
...
function getLoadingNavigation(location$1, submission) {
...
function getSubmittingNavigation(location$1, submission) {
	return {
		state: "submitting",
		location: location$1,
		formMethod: submission.formMethod,
...
Please change the parent <Route path="${parentPath}"> to <Route path="${parentPath === "/" ? "*" : `${parentPath}/*`}">.`);
	...
	if (locationArg) {
		...
		location$1 = parsedLocationArg;
	} else location$1 = locationFromContext;
	let pathname = location$1.pathname || "/";
	...
	if (ENABLE_DEV_WARNINGS) {
		warning(parentRoute || matches != null, `No routes matched location "${location$1.pathname}${location$1.search}${location$1.hash}" `);
		warning(matches == null || matches[matches.length - 1].route.element !== void 0 || matches[matches.length - 1].route.Component !== void 0 || matches[matches.length - 1].route.lazy !== void 0, `Matched leaf route at location "${location$1.pathname}${location$1.search}${location$1.hash}" does not have an element or Component. This means it will render an <Outlet /> with a null value by default resulting in an "empty" page.`);
	}
...
function useBlocker(shouldBlock) {
	let { router: router$1, basename } = useDataRouterContext("useBlocker");
	...
	import_react.useEffect(() => {
		if (blockerKey !== "") router$1.getBlocker(blockerKey, blockerFunction);
	}, [
		router$1,
		blockerKey,
		blockerFunction
...
function useNavigateStable() {
	...
	return import_react.useCallback(async (to, options = {}) => {
		warning(activeRef.current, navigateEffectWarning);
		if (!activeRef.current) return;
		if (typeof to === "number") router$1.navigate(to);
		else await router$1.navigate(to, {
			fromRouteId: id,
			...options
		});
	}, [router$1, id]);
...
function RouterProvider({ router: router$1, flushSync: reactDomFlushSyncImpl, unstable_onError }) {
	...
	let logErrorsAndSetState = import_react.useCallback((newState) => {
		setStateImpl((prevState) => {
			if (newState.errors && unstable_onError) Object.entries(newState.errors).forEach(([routeId, error]) => {
				if (prevState.errors?.[routeId] !== error) unstable_onError(error, {
	...
	let setState = import_react.useCallback((newState, { deletedFetchers, flushSync, viewTransitionOpts }) => {
		...
		let isViewTransitionAvailable = router$1.window != null && router$1.window.document != null && typeof router$1.window.document.startViewTransition === "function";
		...
		if (!viewTransitionOpts || !isViewTransitionAvailable) {
			if (reactDomFlushSyncImpl && flushSync) reactDomFlushSyncImpl(() => logErrorsAndSetState(newState));
			else import_react.startTransition(() => logErrorsAndSetState(newState));
			return;
		}
	...
	import_react.useEffect(() => {
		if (renderDfd && pendingState && router$1.window) {
			...
			let transition2 = router$1.window.document.startViewTransition(async () => {
				import_react.startTransition(() => logErrorsAndSetState(newState));
				await renderPromise;
			});
...
function Routes({ children, location: location$1 }) {
	return useRoutes(createRoutesFromChildren(children), location$1);
}
function Await({ children, errorElement, resolve }) {
	...
	let onError = import_react.useCallback((error, errorInfo) => {
		if (dataRouterContext && dataRouterContext.unstable_onError && dataRouterStateContext) dataRouterContext.unstable_onError(error, {
...
function unflatten(parsed) {
	...
	const startIndex$1 = values.length;
	for (const value of parsed) values.push(value);
	hydrated.length = values.length;
	return hydrate.call(this, startIndex$1);
}
...
function encode(input, options) {
	...
	return new ReadableStream({ async start(controller) {
		const id = flatten.call(encoder$1, input);
		if (Array.isArray(id)) throw new Error("This should never happen");
		if (id < 0) controller.enqueue(textEncoder.encode(`${id}
`));
		else {
			controller.enqueue(textEncoder.encode(`[${encoder$1.stringified.join(",")}]
`));
			lastSentIndex = encoder$1.stringified.length - 1;
		}
		const seenPromises = /* @__PURE__ */ new WeakSet();
		if (Object.keys(encoder$1.deferred).length) {
			let raceDone;
			...
			while (Object.keys(encoder$1.deferred).length > 0) {
				for (const [deferredId, deferred] of Object.entries(encoder$1.deferred)) {
					if (seenPromises.has(deferred)) continue;
					seenPromises.add(encoder$1.deferred[Number(deferredId)] = Promise.race([racePromise, deferred]).then((resolved) => {
						const id2 = flatten.call(encoder$1, resolved);
...
`));
							...
							const values = encoder$1.stringified.slice(lastSentIndex + 1).join(",");
...
`));
						...
						const id2 = flatten.call(encoder$1, reason);
						if (Array.isArray(id2)) {
...
`));
						...
						delete encoder$1.deferred[Number(deferredId)];
					}));
				}
				await Promise.race(Object.values(encoder$1.deferred));
			}
			raceDone();
		}
		await Promise.all(Object.values(encoder$1.deferred));
		controller.close();
	} });
}
...
function getSingleFetchDataStrategyImpl(getRouter, getRouteInfo, fetchAndDecode, ssr, basename, shouldAllowOptOut = () => true) {
	return async (args) => {
		...
		return singleFetchLoaderNavigationStrategy(args, router$1, getRouteInfo, fetchAndDecode, ssr, basename, shouldAllowOptOut);
	};
}
async function singleFetchActionStrategy(args, fetchAndDecode, basename) {
	...
	let result = await actionMatch.resolve(async (handler) => {
		return await handler(async () => {
			let { data: data2, status } = await fetchAndDecode(args, basename, [actionMatch.route.id]);
			actionStatus = status;
...
async function singleFetchLoaderNavigationStrategy(args, router$1, getRouteInfo, fetchAndDecode, ssr, basename, shouldAllowOptOut = () => true) {
	...
	let resolvePromise = Promise.all(args.matches.map(async (m, i) => m.resolve(async (handler) => {
		...
		if (!m.unstable_shouldCallHandler(defaultShouldRevalidate)) {
	...
	if ((!router$1.state.initialized && router$1.state.navigation.state === "idle" || routesParams.size === 0) && !window.__reactRouterHdrActive) singleFetchDfd.resolve({ routes: {} });
	else {
...
function decodeViaTurboStream(body, global$1) {
	return decode$1(body, { plugins: [(type, ...rest) => {
		if (type === "SanitizedError") {
			let [name, message, stack] = rest;
			let Constructor = Error;
			if (name && name in global$1 && typeof global$1[name] === "function") Constructor = global$1[name];
...
function unwrapSingleFetchResult(result, routeId) {
	if ("redirect" in result) {
		let { redirect: location$1, revalidate, reload, replace: replace2, status } = result.redirect;
		throw redirect(location$1, {
			status,
			headers: {
...
function getKeyedLinksForMatches(matches, routeModules, manifest) {
	return dedupeLinkDescriptors(matches.map((match) => {
		...
		return [route && route.css ? route.css.map((href$1) => ({
			rel: "stylesheet",
			href: href$1
		})) : [], module$1?.links?.() || []];
...
function getNewMatchesForLinks(page, nextMatches, currentMatches, manifest, location$1, mode) {
	...
	if (mode === "data") return nextMatches.filter((match, index) => {
		...
		if (match.route.shouldRevalidate) {
			let routeChoice = match.route.shouldRevalidate({
				currentUrl: new URL(location$1.pathname + location$1.search + location$1.hash, window.origin),
...
function RemixRootDefaultHydrateFallback() {
	return /* @__PURE__ */ import_react.createElement(BoundaryShell, {
		title: "Loading...",
		renderScripts: true
	}, ENABLE_DEV_WARNINGS ? /* @__PURE__ */ import_react.createElement("script", { dangerouslySetInnerHTML: { __html: `
              console.log(
...
function getPartialManifest({ sri,...manifest }, router$1) {
	let routeIds = new Set(router$1.state.matches.map((m) => m.route.id));
	let segments = router$1.state.location.pathname.split("/").filter(Boolean);
	let paths = ["/"];
	segments.pop();
	...
	paths.forEach((path) => {
		let matches = matchRoutes(router$1.routes, path, router$1.basename);
...
function useFogOFWarDiscovery(router$1, manifest, routeModules, ssr, routeDiscovery, isSpaMode) {
	import_react.useEffect(() => {
		...
		async function fetchPatches() {
			...
			try {
				await fetchAndApplyManifestPatches(lazyPaths, null, manifest, routeModules, ssr, isSpaMode, router$1.basename, routeDiscovery.manifestPath, router$1.patchRoutes);
			} catch (e) {
...
function PrefetchPageLinks({ page,...linkProps }) {
	...
	let matches = import_react.useMemo(() => matchRoutes(router$1.routes, page, router$1.basename), [
		router$1.routes,
		page,
		router$1.basename
	]);
...
function PrefetchPageLinksImpl({ page, matches: nextMatches,...linkProps }) {
	let location$1 = useLocation();
	let { manifest, routeModules } = useFrameworkContext();
	let { basename } = useDataRouterContext2();
	let { loaderData, matches } = useDataRouterStateContext();
	let newMatchesForData = import_react.useMemo(() => getNewMatchesForLinks(page, nextMatches, matches, manifest, location$1, "data"), [
		page,
		nextMatches,
		matches,
		manifest,
	...
	let newMatchesForAssets = import_react.useMemo(() => getNewMatchesForLinks(page, nextMatches, matches, manifest, location$1, "assets"), [
		page,
		nextMatches,
		matches,
		manifest,
	...
	let dataHrefs = import_react.useMemo(() => {
		if (page === location$1.pathname + location$1.search + location$1.hash) return [];
		let routesParams = /* @__PURE__ */ new Set();
		let foundOptOutRoute = false;
	...
	return /* @__PURE__ */ import_react.createElement(import_react.Fragment, null, dataHrefs.map((href$1) => /* @__PURE__ */ import_react.createElement("link", {
		key: href$1,
		rel: "prefetch",
		as: "fetch",
		href: href$1,
		...linkProps
	})), moduleHrefs.map((href$1) => /* @__PURE__ */ import_react.createElement("link", {
		key: href$1,
		rel: "modulepreload",
		href: href$1,
		...linkProps
...
function Scripts(scriptProps) {
	...
	let { router: router$1, static: isStatic, staticContext } = useDataRouterContext2();
...
${matches.map((match, routeIndex) => {
			...
			let { clientActionModule, clientLoaderModule, clientMiddlewareModule, hydrateFallbackModule, module: module$1 } = manifestEntry;
			let chunks = [
				...clientActionModule ? [{
			...
			if (chunks.length === 1) return `import * as ${routeVarName} from ${JSON.stringify(module$1)};`;
  ...
  ${enableFogOfWar ? `window.__reactRouterManifest = ${JSON.stringify(getPartialManifest(manifest, router$1), null, 2)};` : ""}
...
function RemixRootDefaultErrorBoundary({ error, isOutsideRemixApp }) {
	...
	let heyDeveloper = /* @__PURE__ */ import_react.createElement("script", { dangerouslySetInnerHTML: { __html: `
      ...
      ` } });
	if (isRouteErrorResponse(error)) return /* @__PURE__ */ import_react.createElement(BoundaryShell, { title: "Unhandled Thrown Response!" }, /* @__PURE__ */ import_react.createElement("h1", { style: { fontSize: "24px" } }, error.status, " ", error.statusText), ENABLE_DEV_WARNINGS ? heyDeveloper : null);
	let errorInstance;
...
try {
	if (isBrowser) window.__reactRouterVersion = "7.9.6";
} catch (e) {}
...
var Link = import_react.forwardRef(function LinkWithRef({ onClick, discover = "render", prefetch = "none", relative, reloadDocument, replace: replace2, state, target, to, preventScrollReset, viewTransition,...rest }, forwardedRef) {
	...
	let href$1 = useHref(to, { relative });
	...
	let link = /* @__PURE__ */ import_react.createElement("a", {
		...rest,
		...prefetchHandlers,
		href: absoluteHref || href$1,
		onClick: isExternal || reloadDocument ? onClick : handleClick,
		ref: mergeRefs(forwardedRef, prefetchRef),
		target,
		"data-discover": !isAbsolute && discover === "render" ? "true" : void 0
	});
	return shouldPrefetch && !isAbsolute ? /* @__PURE__ */ import_react.createElement(import_react.Fragment, null, link, /* @__PURE__ */ import_react.createElement(PrefetchPageLinks, { page: href$1 })) : link;
});
...
var NavLink = import_react.forwardRef(function NavLinkWithRef({ "aria-current": ariaCurrentProp = "page", caseSensitive = false, className: classNameProp = "", end = false, style: styleProp, to, viewTransition, children,...rest }, ref) {
	...
	let locationPathname = location$1.pathname;
...
var Form = import_react.forwardRef(({ discover = "render", fetcherKey, navigate, reloadDocument, replace: replace2, state, method = defaultMethod, action, onSubmit, relative, preventScrollReset, viewTransition,...props }, forwardedRef) => {
...
function ScrollRestoration({ getKey, storageKey,...props }) {
	...
	let ssrKey = import_react.useMemo(() => {
		...
		let userKey = getScrollRestorationKey(location$1, matches, basename, getKey);
		return userKey !== location$1.key ? userKey : null;
	}, []);
	...
	return /* @__PURE__ */ import_react.createElement("script", {
		...props,
		suppressHydrationWarning: true,
		dangerouslySetInnerHTML: { __html: `(${restoreScroll})(${JSON.stringify(storageKey || SCROLL_RESTORATION_STORAGE_KEY)}, ${JSON.stringify(ssrKey)})` }
	});
}
...
function useLinkClickHandler(to, { target, replace: replaceProp, state, preventScrollReset, relative, viewTransition } = {}) {
	...
	return import_react.useCallback((event) => {
		if (shouldProcessLinkClick(event, target)) {
			event.preventDefault();
			navigate(to, {
				replace: replaceProp !== void 0 ? replaceProp : createPath(location$1) === createPath(path),
				state,
				preventScrollReset,
				relative,
				viewTransition
...
function useSearchParams(defaultInit) {
	...
	let location$1 = useLocation();
	let searchParams = import_react.useMemo(() => getSearchParamsForLocation(location$1.search, hasSetSearchParamsRef.current ? null : defaultSearchParamsRef.current), [location$1.search]);
	let navigate = useNavigate();
...
function useSubmit() {
	...
	return import_react.useCallback(async (target, options = {}) => {
		...
		if (options.navigate === false) {
			...
			await router$1.fetch(key, currentRouteId, options.action || action, {
		...
		} else await router$1.navigate(options.action || action, {
			preventScrollReset: options.preventScrollReset,
			formData,
			body,
			formMethod: options.method || method,
...
function useFetcher({ key } = {}) {
	let { router: router$1 } = useDataRouterContext3("useFetcher");
	...
	import_react.useEffect(() => {
		...
		return () => router$1.deleteFetcher(fetcherKey);
	}, [router$1, fetcherKey]);
	let load = import_react.useCallback(async (href$1, opts) => {
		invariant(routeId, "No routeId available for fetcher.load()");
		await router$1.fetch(fetcherKey, routeId, href$1, opts);
	}, [
		fetcherKey,
		routeId,
		router$1
	...
	let unstable_reset = import_react.useCallback((opts) => router$1.resetFetcher(fetcherKey, opts), [router$1, fetcherKey]);
...
function getScrollRestorationKey(location$1, matches, basename, getKey) {
	let key = null;
	if (getKey) if (basename !== "/") key = getKey({
		...location$1,
		pathname: stripBasename(location$1.pathname, basename) || location$1.pathname
	}, matches);
	else key = getKey(location$1, matches);
	if (key == null) key = location$1.key;
	return key;
}
function useScrollRestoration({ getKey, storageKey } = {}) {
	...
	usePageHide(import_react.useCallback(() => {
		if (navigation.state === "idle") {
			let key = getScrollRestorationKey(location$1, matches, basename, getKey);
	...
	if (typeof document !== "undefined") {
		...
		import_react.useLayoutEffect(() => {
			let disableScrollRestoration = router$1?.enableScrollRestoration(savedScrollPositions, () => window.scrollY, getKey ? (location2, matches2) => getScrollRestorationKey(location2, matches2, basename, getKey) : void 0);
		...
		import_react.useLayoutEffect(() => {
			...
			try {
				if (location$1.hash) {
					let el = document.getElementById(decodeURIComponent(location$1.hash.slice(1)));
					if (el) {
						el.scrollIntoView();
						return;
					}
				...
				warning(false, `"${location$1.hash.slice(1)}" is not a decodable element ID. The view will not scroll to it.`);
			}
...
function StaticRouterProvider({ context, router: router$1, hydrate: hydrate2 = true, nonce }) {
	invariant(router$1 && context, "You must provide `router` and `context` to <StaticRouterProvider>");
...
//#endregion
//#region node_modules/.pnpm/cookie@1.0.2/node_modules/cookie/dist/index.js
var require_dist = /* @__PURE__ */ __commonJS({ "node_modules/.pnpm/cookie@1.0.2/node_modules/cookie/dist/index.js": ((exports) => {
...
var require_set_cookie = /* @__PURE__ */ __commonJS({ "node_modules/.pnpm/set-cookie-parser@2.7.2/node_modules/set-cookie-parser/lib/set-cookie.js": ((exports, module) => {
	...
	module.exports.splitCookiesString = splitCookiesString$1;
}) });

//#endregion
//#region node_modules/.pnpm/react-router@7.9.6_react-do_d4a3e4c27e0cb30e84e7f4e67d54cb39/node_modules/react-router/dist/development/chunk-G3INQAYP.mjs
/**
* react-router v7.9.6
*
* Copyright (c) Remix Software Inc.
...
function ServerRouter({ context, url, nonce }) {
	...
	let router$1 = createStaticRouter(routes, context.staticHandlerContext);
	...
	} }, /* @__PURE__ */ import_react.createElement(RemixErrorBoundary, { location: router$1.state.location }, /* @__PURE__ */ import_react.createElement(StaticRouterProvider, {
		router: router$1,
...
async function callRouteHandler(handler, args) {
	let result = await handler({
		request: stripRoutesParam(stripIndexParam$1(args.request)),
		params: args.params,
...
function stripIndexParam$1(request) {
	let url = new URL(request.url);
	let indexValues = url.searchParams.getAll("index");
	url.searchParams.delete("index");
	let indexValuesToKeep = [];
...
function href(path, ...args) {
	let params = args[0];
	let result = path.replace(/\/*\*?$/, "").replace(/\/:([\w-]+)(\?)?/g, (_, param, questionMark) => {
...
function RSCDefaultRootErrorBoundaryImpl({ error, renderAppShell }) {
	...
	}, /* @__PURE__ */ import_react.createElement("h1", { style: { fontSize: "24px" } }, error.status, " ", error.statusText), ENABLE_DEV_WARNINGS ? heyDeveloper : null);
	let errorInstance;
	if (error instanceof Error) errorInstance = error;
	else {
		let errorString = error == null ? "Unknown Error" : typeof error === "object" && "toString" in error ? error.toString() : JSON.stringify(error);
...
async function routeRSCServerRequest({ request, fetchServer, createFromReadableStream, renderHTML, hydrate: hydrate$1 = true }) {
	const url = new URL(request.url);
	const respondWithRSCPayload = isReactServerRequest(url) || isManifestRequest(url) || request.headers.has("rsc-action-id");
	...
	if (hydrate$1) serverResponseB = serverResponse.clone();
	...
	try {
		...
		if (!hydrate$1) return new Response(html, {
...
function RSCStaticRouter({ getPayload }) {
	...
	const router$1 = createStaticRouter(payload.matches.reduceRight((previous, match) => {
		const route = {
			id: match.id,
			action: match.hasAction || !!match.clientAction,
...
function getHydrationData({ state, routes, getRouteInfo, location: location$1, basename, isSpaMode }) {
	let hydrationData = {
		...state,
		loaderData: { ...state.loaderData }
	};
	let initialMatches = matchRoutes(routes, location$1, basename);
...
//#endregion
//#region node_modules/.pnpm/react-router@7.9.6_react-do_d4a3e4c27e0cb30e84e7f4e67d54cb39/node_modules/react-router/dist/development/dom-export.mjs
/**
* react-router v7.9.6
*
* Copyright (c) Remix Software Inc.
...
//#endregion
//#region node_modules/.pnpm/react-router-dom@7.9.6_reac_b967de791348b38b12787b51200750ef/node_modules/react-router-dom/dist/index.mjs
/**
